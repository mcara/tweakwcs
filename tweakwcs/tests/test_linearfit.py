...
if linalg._MAX_LINALG_TYPE is not None:
    _ATOL = 10 * _LARGE_SAMPLE_SIZE * np.sqrt(np.finfo(linalg._MAX_LINALG_TYPE).eps)
else:
...
@pytest.fixture(
    scope="function",
    params=["shifts", "rshift", "rscale", "rscale-flip-x", "rscale-flip-y", "affine"],
)
...
@pytest.fixture(scope="function", params=list(product(*(2 * [[False, True]]))))
def weight_data(request):
    ...
    if not any(request.param):
        ...
        nbduv = nbd - nbdxy
        idx_xy = (np.random.choice(np.arange(_LARGE_SAMPLE_SIZE), nbdxy, replace=False),)
        idx_uv = (np.random.choice(np.arange(_LARGE_SAMPLE_SIZE), nbduv, replace=False),)
        ...
        idx = np.random.choice(np.arange(_LARGE_SAMPLE_SIZE), nbd, replace=False)
        idx_xy = (idx,)
        idx_uv = (np.array([], dtype=int),)
        wxy = np.random.random(_LARGE_SAMPLE_SIZE)
        wxy[idx_xy] = 0.0
...
@pytest.fixture(
    scope="function", params=[linearfit.fit_shifts, linearfit.fit_rscale, linearfit.fit_general]
)
...
def test_build_fit_matrix_generalized(rot, scale):
    ...
    # check scale:
    assert np.allclose(np.sqrt(np.sum(m**2, axis=0)), scale, rtol=0, atol=_ATOL)
    ms = np.diag(scale)

    # check rotations:
    mr = linearfit.build_fit_matrix(rot, 1)
    mrinv = linearfit.build_fit_matrix(rot[::-1], 1).T
    assert np.allclose(np.linalg.det(mr) * i, np.dot(mr, mrinv), rtol=0, atol=_ATOL)
...
@pytest.mark.parametrize(
    "nclip, sigma, clip_accum, weights, noise",
    [
...
def test_iter_linear_fit_special_cases(ideal_large_data, nclip, sigma, clip_accum, weights, noise):
...
def test_iter_linear_fit_1point(weights):
    ...
    fit = linearfit.iter_linear_fit(xy, xy + shifts, wxy=wxy, wuv=wuv, fitgeom="shift", nclip=0)
...
def test_iter_linear_fit_fitgeom_clip_all_data(ideal_large_data):
    ...
    fit = linearfit.iter_linear_fit(xy, uv, wxy, wuv, fitgeom=fitgeom, sigma=1e-50, nclip=100)
...
@pytest.mark.parametrize(
    "fit_function",
    [
        linearfit.fit_rscale,
        linearfit.fit_general,
    ],
)
...
@pytest.mark.parametrize("clip_accum, noise", list(product(*(2 * [[False, True]]))))
def test_iter_linear_fit_clip_style(ideal_large_data, weight_data, clip_accum, noise):
    ...
    fit = linearfit.iter_linear_fit(
        xy, uv, wxy=wxy, wuv=wuv, fitgeom=fitgeom, sigma=2, clip_accum=clip_accum, nclip=nclip
    )
    ...
    if nclip:
        ...
        assert fit["eff_nclip"] == 0
        assert fit["fitmask"].sum(dtype=int) == npts - np.union1d(idx_xy[0], idx_uv[0]).size
...
def test_levmar2x2_multivariate():
    inputs = [np.array([10.0, 10.0, 20.0, 20.0]), np.array([10.0, 20.0, 20.0, 10.0])]
    outputs = [
    ...
    assert np.allclose(finfo.parameters, np.array([4.3, -7.1, 45.0]), rtol=1e-5, atol=1e-5)
